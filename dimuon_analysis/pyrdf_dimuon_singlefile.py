...
def dimuonSpectrum(df):
    ...
    with open("pyrdf_dimuon_analysistimes.csv", "a+") as f:
...
df = PyRDF.RDataFrame("Events", officialdataset)
...
for i in range(1):
    ...
    PyRDF.current_backend.sparkContext.dump_profiles(os.getcwd()+"/spark_profile")

    with open("pyrdf_dimuon_singlefile.csv", "a+") as f:
