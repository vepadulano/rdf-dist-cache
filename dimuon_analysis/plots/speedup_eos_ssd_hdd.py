...
xrdssdspeedup = [ 1. / (time / xrdssdtimes[0]) for time in xrdssdtimes ]

# Best times with XRootD proxies on all `sg` machines
# caching on /htda/cache/vpadulan

xrdhddtimes = [4133.3, 2136.8, 1225.11, 849.08, 1864.79, 1252.24, 1013.45]
xrdhddspeedup = [ 1. / (time / xrdhddtimes[0]) for time in xrdhddtimes ]
...
xrdgraph = ROOT.TGraph(numpoints)
for i in range(numpoints): # only start plotting from 16 cores
...
xrdhddgraph = ROOT.TGraph(numpoints)
for i in range(numpoints): # only start plotting from 16 cores
    xrdhddgraph.SetPoint(i, cores[i], xrdhddspeedup[i])
...
c.Draw()
c.SaveAs("speedup_eos_ssd_hdd.png")